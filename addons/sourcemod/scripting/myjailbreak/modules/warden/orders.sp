...
public void Orders_OnMapStart()
{
	...
	if (!hFile)
	{
	//	LogError("MyJailbreak Warden - Can't open File: %s", g_sMenuFile);
		return;
	}
	...
	{
	//	LogError("MyJailbreak Warden - Can't read %s correctly! (ImportFromFile)", g_sMenuFile);
		return;
	}
	if (!kvMenu.GotoFirstSubKey())
	{
	//	LogError("MyJailbreak Warden - Can't read %s correctly! (GotoFirstSubKey)", g_sMenuFile);
	}
	do
	{
		char sSound[64];
...
//	else LogError("MyJailbreak Warden - Can't open File: %s", g_sMenuFile);
}
