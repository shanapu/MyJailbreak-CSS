...
public void OnPluginStart() 
{
	...
	gc_sModelPathWarden = AutoExecConfig_CreateConVar("sm_warden_model_path", "models/player/vad36mk9/stryker.mdl", "Path to the model for warden.");
...
//	HandCuffs_OnPluginStart();
	...
	HookEvent("round_start", Event_PostRoundStart, EventHookMode_Post);
...
void ResetModel(int client)
{
	if (!gc_bModel.BoolValue)
		return;

	SetEntityModel(client, g_sModelPathPrevious);
}
...
public APLRes AskPluginLoad2(Handle myself, bool late, char[] error, int err_max)
{
	...
	{
		SetFailState("Game is not supported. CSS ONLY");
	}
